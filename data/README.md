...
- `group_id`: a caption/source/near-duplicate group that must not cross a custom split;
- `source_id`: stable source identifier when the dataset provides one.
...
1. **Defactify Image Dataset** is the starting train/validation/internal-test benchmark.
   ...
   that split, `01_data_audit_and_splits.ipynb` must audit caption and perceptual-hash overlap.
