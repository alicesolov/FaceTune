"""Create a metadata-only DANI catalogue without initialising Torch.

The process-local USE_TORCH=0 setting is established before importing the scanner module. It does
not modify the environment used by model training or any other project command.
"""
...
# Keep Hub cache and lock state inside the ignored project artifact directory.
...
from ai_image_detector.dani import PINNED_REVISION, scan_metadata


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Column-scan DANI metadata only; the image field is never requested. "
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("data/processed/dani_metadata"),
