...
% Figures are rebuilt from frozen local artifacts by scripts/build_report_figures.py.
...
Figure~\ref{fig:split-composition} visualises the deliberate four-to-one observation ratio. The
training sampler later balances binary optimisation within each parent; the reported validation
and test metrics retain the full declared evaluation distribution.

\begin{center}
\centering
\includegraphics[width=0.94\textwidth]{../../output/report_figures/dataset_split_composition.png}
\captionof{figure}{Real and AI-generated image counts in the parent-disjoint split. Every parent contributes
one real row and four generated rows.}
\label{fig:split-composition}
\end{center}

Figure~\ref{fig:parent-example} gives a qualitative view of one complete parent group. To avoid
illustration cherry-picking, the report builder selects the lowest numeric COCO parent identifier
among complete validation groups with readable files. It does not inspect model predictions or
test data. Image-conditioned outputs remain visually close to the real parent, whereas the two
text-to-image outputs reconstruct the scene more freely.

\begin{center}
\centering
\includegraphics[width=\textwidth]{../../output/report_figures/dani_parent_group_examples.png}
\captionof{figure}{Deterministically selected DANI validation group \texttt{coco-parent:785}: one real COCO
photograph and four declared generated conditions. The pinned manifest records the real image as
CC BY 2.0; the DANI subset is used under its non-commercial research licence \cite{dani2026card}.}
\label{fig:parent-example}
\end{center}
...
\begin{center}
\centering
\includegraphics[width=\textwidth]{../../output/report_figures/experimental_pipeline.png}
\captionof{figure}{Evidence flow from pinned metadata to the one-time frozen test. The internal test is not
used during representation, seed, checkpoint, or threshold selection.}
\label{fig:pipeline}
\end{center}

Figure~\ref{fig:rgb-fft-example} shows that FFT magnitude is not an alternative picture rendering
but a frequency representation of the same common raster. Bright central energy corresponds to
low spatial frequencies, while directional lines and off-centre structure encode repeated edges
and textures. The two spectra are descriptive examples only; no decision is made by visual
inspection of this pair.

\begin{center}
\centering
\includegraphics[width=0.72\textwidth]{../../output/report_figures/rgb_fft_examples.png}
\captionof{figure}{The common RGB raster and shifted log FFT magnitude for the real and SDXL-I2I rows of the
deterministically selected validation parent. Each spectrum is robustly scaled for display.}
\label{fig:rgb-fft-example}
\end{center}
...
\begin{center}
\centering
\includegraphics[width=0.96\textwidth]{../../output/report_figures/validation_seed_comparison.png}
\captionof{figure}{Matched validation ROC-AUC and balanced accuracy for every predeclared seed. The graph
makes both the consistent FFT advantage and the seed-42 degradation visible.}
\label{fig:seed-comparison}
\end{center}
...
\begin{center}
\centering
\includegraphics[width=0.62\textwidth]{../../output/report_figures/clean_confusion_matrix.png}
\captionof{figure}{Clean internal-test confusion matrix at the validation-selected threshold. Percentages
are normalised within each actual class so the four-to-one observation ratio does not hide class
recall.}
\label{fig:confusion}
\end{center}
...
\begin{center}
\centering
\includegraphics[width=0.88\textwidth]{../../output/report_figures/generator_performance.png}
\captionof{figure}{Clean internal-test performance by generated condition against the same 222 real
parents. T2I is materially easier for the frozen model than I2I and TI2I.}
\label{fig:generator-performance}
\end{center}
...
\begin{center}
\captionof{table}{Aggregate robustness results.}
...
\begin{center}
\centering
\includegraphics[width=0.96\textwidth]{../../output/report_figures/robustness_metrics.png}
\captionof{figure}{ROC-AUC and balanced accuracy under the seven predeclared conditions, with the checkpoint
and threshold fixed. The below-chance ordering after resize and blur is a central negative result.}
...
Report figures & \path{scripts/build_report_figures.py}; deterministic selection record in \path{output/report_figures/report_figures_manifest.json}\\
