...
def radial_preprocessing_metadata(protocol: str) -> dict[str, object]:
    ...
    if protocol in {
        ...
        elif protocol == DANI_HIGHRES_PREPROCESSING_PROTOCOL:
            metadata["crop_policy"] = "exact_square_source_all_splits"
        else:
...
def radial_features(
    ...
    for path in tqdm(frame["path"], desc="radial FFT features"):
        with Image.open(Path(path)) as image:
            ...
            elif preprocessing_protocol == DANI_HIGHRES_PREPROCESSING_PROTOCOL:
                image = dani_highres_rasterize(image, size=image_size)
